...
export default async function handler(req, res) {
  ...
  console.log(
    "SCRAPELLM:",
    process.env.SCRAPELLM_API_KEY ? "Loaded" : "Missing"
  );
  ...
  const diagnostics = {
    ...
    authDomain: process.env.NEXT_PUBLIC_FIREBASE_AUTH_DOMAIN
      ? "Loaded"
      : "Missing",
    scrapeLlmKey: process.env.SCRAPELLM_API_KEY
      ? "Loaded"
      : "Missing",
  };
  ...
  const testQuery =
    "Crypto market sentiment structural overview";

  const llmTestUrl =
    `https://api.scrapellm.com/scrapers/chatgpt?prompt=${encodeURIComponent(
      testQuery
    )}`;

  try {
    const [firebaseResult, llmResult] =
    ...
    if (firebaseResult.status === "fulfilled") {
      ...
      if (
        ...
        firebaseStatus = {
          connected: true,
          details:
            "Firebase endpoint reached successfully.",
        };
      } else {
        firebaseStatus = {
          connected: false,
          details:
            errorMsg || fbResponse.statusText,
        };
      }
    } else {
      firebaseStatus.details = String(
    ...
    if (llmResult.status === "fulfilled") {
      ...
      if (llmResponse.ok) {
        const llmData = await llmResponse
          .json()
          .catch(() => ({}));
